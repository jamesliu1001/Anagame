...
#-> tuple[bool, str, str]
def basic_checks(word1:str, word2:str):
    ...
    if word1 == word2: return [False, word1, word2]

    if len(word1) <=2 or len(word2) <=2 or len(word1) != len(word2): return [False, word1 , word2]

    if word1.isalpha() == False or word2.isalpha() == False:
...
def is_anagram_exhaustive(word1:str, word2:str)->bool:
    ...
    conditions = basic_checks(word1, word2)
    ...
    perms =  list(itertools.permutations(word1))

    if tuple(word2) in perms:
        return True 
...
def is_anagram_prime_hash(word1:str, word2:str)->bool:
    ...
    counter2 = 1
    for n in word1: 
        counter1 *= ch_to_prime[n]
    
    for j in word2:
        counter2 *= ch_to_prime[j]
